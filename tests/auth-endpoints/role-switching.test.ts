...
import { isAuthServerAvailable, authenticateUser } from "../setup/auth-server";
...
describe("Role Switching Endpoint", () => {
  ...
  describe("Role Validation", () => {
    ...
    it("should allow switching to a role user has", async () => {
      ...
      expect(data.activeRole).toBe("receptionist");
      expect(data.availableRoles).toContain("physician");
      expect(data.availableRoles).toContain("receptionist");
      expect(data.message).toBe("Role switched successfully. Please refresh your session to get a new token.");
    });
    ...
    it("should allow switching back to original role", async () => {
      ...
      const data = await response.json();
      expect(data.success).toBe(true);
      expect(data.activeRole).toBe("physician");
      expect(data.availableRoles).toContain("physician");
      expect(data.availableRoles).toContain("receptionist");
    });
  });
  ...
  describe("Single Role Users", () => {
    it("should allow single-role user to switch to their own role", async () => {
      ...
      const data = await response.json();
      expect(data.success).toBe(true);
      expect(data.activeRole).toBe("org_admin");
      expect(data.availableRoles).toEqual(["org_admin"]);
    });

    it("should reject single-role user attempting to switch to different role", async () => {
      // org1Physician has only "physician" role
      const signInResponse = await fetch(`${AUTH_SERVER_URL}/api/auth/sign-in/email`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  describe("Database Persistence", () => {
    it("should persist activeRole change in database", async () => {
      ...
      expect(payload["https://hasura.io/jwt/claims"]["x-hasura-default-role"]).toBe("receptionist");
    });
  });
  ...
  describe("Multiple Role Scenarios", () => {
    it("should handle user with multiple roles correctly", async () => {
      ...
      expect(receptionistData.activeRole).toBe("receptionist");
      ...
      expect(physicianData.activeRole).toBe("physician");

      // Both responses should show all available roles
      expect(receptionistData.availableRoles).toContain("physician");
      expect(receptionistData.availableRoles).toContain("receptionist");
      expect(physicianData.availableRoles).toContain("physician");
      expect(physicianData.availableRoles).toContain("receptionist");
    });
  });
});