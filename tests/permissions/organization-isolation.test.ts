...
describe('Organization Isolation', () => {
  ...
  describe('Patient Access Control', () => {
    it('org_admin can only access patients from their organization', async () => {
      ...
      expect(org1Patients.patient.every((p) => p.organization_id === TestDataIds.organizations.org1)).toBe(true);
      ...
      expect(org2Patients.patient.every((p) => p.organization_id === TestDataIds.organizations.org2)).toBe(true);
    });
    ...
    it('physician can only access patients from their organization', async () => {
      ...
      expect(org1PhysicianPatients.patient.every((p) => p.organization_id === TestDataIds.organizations.org1)).toBe(true);
    });
    ...
    it('receptionist can only access patients from their organization', async () => {
      ...
      expect(org1ReceptionistPatients.patient.every((p) => p.organization_id === TestDataIds.organizations.org1)).toBe(true);
    });
  });
  ...
  describe('Organization Access Control', () => {
    it('org_admin can only access their own organization', async () => {
      ...
      const org1Result = await clients.org1Admin.request<{organization: Array<{id: string, name: string}>}>(`
        query {
          organization {
            id
            name
      ...
      expect(org1Result.organization[0].id).toBe(TestDataIds.organizations.org1);

      const org2Result = await clients.org2Admin.request<{organization: Array<{id: string, name: string}>}>(`
        query {
          organization {
            id
            name
      ...
      expect(org2Result.organization[0].id).toBe(TestDataIds.organizations.org2);
    });

    it('physician can only access their organization', async () => {
      const result = await clients.org1Physician.request<{organization: Array<{name: string}>}>(`
        query {
          organization {
            name
          }
    ...
    it('receptionist can only access their organization', async () => {
      const result = await clients.org1Receptionist.request<{organization: Array<{name: string}>}>(`
        query {
          organization {
            name
          }
    ...
    it('org_admin can update their own organization', async () => {
      ...
      const result = await clients.org1Admin.request<{update_organization_by_pk: {id: string, phone: string}}>(`
        mutation {
  ...
  describe('Patient Registration Access Control', () => {
    ...
    it('physician can only see registrations assigned to them or created by them', async () => {
  ...
  describe('Patient Consent Access Control', () => {
    it('org_admin can access patient_consent within their organization', async () => {
      ...
      if (result.patient_consent.length > 0) {
        expect(result.patient_consent.every(c => 
          c.organization_id === TestDataIds.organizations.org1
        )).toBe(true);
      }
    });
  });
  ...
  describe('Questionnaire Response Access Control', () => {
    it('org_admin can access questionnaire_response within their organization', async () => {
      ...
      if (result.questionnaire_response.length > 0) {
        expect(result.questionnaire_response.every(qr => 
          qr.organization_id === TestDataIds.organizations.org1
        )).toBe(true);
      }
    });
  });
