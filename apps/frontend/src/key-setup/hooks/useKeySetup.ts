...
export function useKeySetup() {
  ...
  const organizationId = (session?.user as any)?.organizationId || 'org_unknown';
  ...
  const organizationPublicKey = organizationData?.organization_by_pk?.public_key_pem;
  const organizationName = organizationData?.organization_by_pk?.name || `Organization ${organizationId}`;

  // Memoize the validation complete callback to prevent infinite re-renders
  const handleValidationComplete = useCallback(
    (_isValid: boolean, validationData?: {
      hasLocalKey: boolean;
    ...
    [orgLoading, organizationId, organizationName, activeRole, organizationPublicKey, actions]
  );
