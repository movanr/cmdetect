...
export async function storePrivateKey(privateKey: string, password: string): Promise<void> {
  try {
    ...
    const salt = crypto.getRandomValues(new Uint8Array(CRYPTO_CONSTANTS.KEY_DERIVATION_SALT_SIZE));
...
export async function loadPrivateKey(password: string): Promise<string> {
  try {
    ...
    const cryptoError: CryptoError = {
      ...
      code: error instanceof Error && error.message.includes('password') ? 'INVALID_PASSWORD' : 'STORAGE_LOAD_FAILED'
    };
    throw cryptoError;
  }
}
...
export async function hasStoredPrivateKey(): Promise<boolean> {
  try {
    ...
    return await new Promise<boolean>((resolve, reject) => {
      ...
      request.onerror = () => reject(new Error('Failed to check for stored key'));
    });
  } catch (error) {
    return false;
  }
